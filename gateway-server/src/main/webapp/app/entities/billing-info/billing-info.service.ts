...
export class BillingInfoService {
  ...
  create(billingInfo: IBillingInfo): Observable<EntityResponseType> {
    return this.http.post<IBillingInfo>(this.resourceUrl, billingInfo, { observe: 'response' });
  }

  update(billingInfo: IBillingInfo): Observable<EntityResponseType> {
    return this.http.put<IBillingInfo>(this.resourceUrl, billingInfo, { observe: 'response' });
  }

  find(id: string): Observable<EntityResponseType> {
    return this.http.get<IBillingInfo>(`${this.resourceUrl}/${id}`, { observe: 'response' });
  }

  query(req?: any): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http.get<IBillingInfo[]>(this.resourceUrl, { params: options, observe: 'response' });
  }
