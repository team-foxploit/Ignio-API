...
export class BillingInfoComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.billingInfoService
      ...
      .subscribe(
        (res: HttpResponse<IBillingInfo[]>) => this.paginateBillingInfos(res.body, res.headers),
  ...
  trackId(index: number, item: IBillingInfo) {
    return item.id;
  }

  registerChangeInBillingInfos() {
    this.eventSubscriber = this.eventManager.subscribe('billingInfoListModification', response => this.loadAll());
  }
  ...
  protected paginateBillingInfos(data: IBillingInfo[], headers: HttpHeaders) {
